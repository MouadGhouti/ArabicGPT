...
def set_batch_size(B=32, T=1024, ddp_world_size=1):
    total_batch_size = B * T * ddp_world_size
    assert total_batch_size % (B * T * ddp_world_size) == 0, "make sure total_batch_size is divisible by B * T * ddp_world_size"
    grad_accum_steps = total_batch_size // (B * T * ddp_world_size)
    return B ,T, total_batch_size, grad_accum_steps
...
B ,T, total_batch_size, grad_accum_steps = set_batch_size(B=32, T=1024, ddp_world_size=ddp_world_size)#CHANGE BATCH SIZE HERE

max_lr = 6e-4
min_lr = max_lr * 0.1
warmup_steps = 715
num_epochs = 1
num_steps = get_num_steps(total_batch_size, total_number_of_tokens=4e9)#CHANGE TOTAL NUMBER OF TOKENS HERE


max_steps = num_steps * num_epochs #if data is 10B tokens and batch size 0.5M tokens (10B in 2 hours on 8 GPUs)


enc = PreTrainedTokenizerFast(tokenizer_file="./Dataset/BPE_Moshakal_Tokenizer/TBPE_tokenizer_32.0K.json")
